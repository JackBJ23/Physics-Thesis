...
def get_gif(ncurves, x1, x2, x3, x4, duration_t, bond1, bond2, bond3, bond4, namefile):
  ...
  for i, y in enumerate(ψ):
      ...
      plt.plot(x, ψ_r[i], label=f'Classical {i}', color='green')
...
#for the case N=16, and there is no SC solution
def get_gif16(ncurves, x1, x2, x3, x4, duration_t, bond1, bond2, bond3, bond4, namefile):
  filenames = []
  for i, y in enumerate(ψ):
      #if i==16: break
      plt.figure(figsize=(8, 6))
      ...
      plt.plot(x, mps_fcts[0][i], label=f'MPS {i}, $\\chi={bond1}$', color='orange')
      if ncurves>=2: plt.plot(x, mps_fcts[1][i], label=f'MPS {i}, $\\chi={bond2}$', color='red')
      if ncurves>=3: plt.plot(x, mps_fcts[2][i], label=f'MPS {i}, $\\chi={bond3}$', color='pink')
      if ncurves>=4: plt.plot(x, mps_fcts[3][i], label=f'MPS {i}, $\\chi={bond4}$', color='blue')
...
def get_gif16(ncurves, x1, x2, x3, x4, duration_t, bond1, bond2, bond3, bond4, namefile):
  ...
  # Create an animated gif
  with imageio.get_writer(namefile, mode='I', duration=duration_t*30) as writer:
      for filename in filenames:
          image = imageio.imread(filename)
          writer.append_data(image)
